...
namespace FBReader.AppServices.ViewModels.Pages
{
    public class MainPageViewModel: Conductor<Screen>.Collection.OneActive
    {
        ...
        public MainPageViewModel(INavigationService navigationService, UserLibraryViewModel userLibaryViewModel, CatalogsViewModel catalogsViewModel, DataBaseInitializer dbInitialzier, CopsPageViewModel copsPageViewModel)
        {
            _navigationService = navigationService;
            _dbInitialzier = dbInitialzier;
            UserLibraryViewModel = userLibaryViewModel;
            CatalogsViewModel = catalogsViewModel;
            CopsPageViewModel = copsPageViewModel;
        }
        ...
        public CopsPageViewModel CopsPageViewModel
        {
            get;
            set;
        }
        ...
        public void OnBackPressed(CancelEventArgs cancelEventArgs)
        {
            if (CopsPageViewModel == null)
            {
                return;
            }

            if (!CopsPageViewModel.CanGoToPreviousLevel)
            {
                return;
            }

            cancelEventArgs.Cancel = true;
            CopsPageViewModel.GoBack();
        }
    }
}